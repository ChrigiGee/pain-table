...
uses
  ...
  ZDataset, Forms, Controls, Graphics, Dialogs, ComCtrls, DBCtrls, DBGrids,
  Buttons, EditBtn, Calendar, XMLPropStorage, Menus;
...
uses
 {implementiert unit1 für die form1
  ...
  monat_jahr_unit, TLoggerUnit, TLevelUnit, TFileAppenderUnit; // ,
// UpDateMode;
{ TForm2 }
...
begin
  ...
  // ZQuery1.DataSource..DataBase.DatabaseName := 'Form1.ZConnection1.DatabaseName';
  ZQuery1.ReadOnly := False;
  // SQLQuery1.FileName:='migraenetagebuch.sql3db';
  ZQuery1.SQL.Text := 'Select * FROM tblMedikamente';
  ZQuery1.ReadOnly := False;
  // ZQuery1.Options. := upWhereAll;
  ZQuery1.Open;
  ZQuery1.Active := True;
...
begin
  ...
  ZQuery1.Close;
  ZQuery1.active := False;
  //ZQuery1.DataBase.DatabaseName := 'Form1.SQLite3Connection1.DatabaseName';
  ZQuery1.ReadOnly := False;
  // SQLQuery1.FileName:='migraenetagebuch.sql3db';
  ZQuery1.SQL.Text := 'Select * FROM tblMedikamente';
  // ZQuery1.UpdateMode := upWhereAll;
  ZQuery1.ReadOnly := False;
  ZQuery1.Open;
  ZQuery1.Active := True;
...
begin
  ...
  ZQuery1.ReadOnly := False;
  Form2.ZQuery1.ApplyUpdates;
  ZQuery1.ReadOnly := True;
  Form2.ZQuery1.active := False;
  ZQuery1.Close;
...
begin
  ...
  ZQuery1.ReadOnly := False;
  Form1.ZConnection1.AutoCommit := True;
  // Options.doUpdateMasterFirst:=True; // := umUpdateChanged;
  Form2.ZQuery1.ApplyUpdates;
  ZQuery1.ReadOnly := True;
  Form2.ZQuery1.active := False;
  ZQuery1.Close;
...
begin
  ZQuery1.SQL.Text := 'UPDATE * FROM tblMedikamente';
end;
